...
function buildRows(users: User[], filter: Filter): LeaderRow[] {
  ...
  for (const user of users) {
    ...
    rows.push({
      user, startWeight: startEntry.weight, currentWeight: latest.weight,
      lost, lostPct, goalPct, entries: all.length, lastDate: latest.date, isNewcomer,
    });
  }
...
function BoardInner({ session }: { session: Session }) {
  ...
  useEffect(() => {
    ...
    setRows(buildRows(users, filter));
    setLoading(false);
  }, [filter]);
          ...
          {rows.slice(0, 3).map((row, i) => (
            <div key={row.user.id}
              className={`relative overflow-hidden rounded-2xl p-5 border ${
                i === 0
                  ? "bg-gradient-to-br from-yellow-400/20 to-amber-500/10 border-yellow-400/30 dark:border-yellow-500/20"
                  : i === 1
                  ? "bg-gradient-to-br from-zinc-300/20 to-zinc-400/10 border-zinc-300/30 dark:border-zinc-600/30"
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="w-10 h-10 rounded-xl bg-white/50 dark:bg-zinc-800 flex items-center justify-center text-xl shadow-sm">
              ...
              <div className="text-3xl font-extrabold text-green-500">
                {row.lost >= 0 ? "-" : "+"}{Math.abs(row.lost).toFixed(1)} кг
              </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((row, i) => {
                  ...
                  return (
                    <tr key={row.user.id}
                      className={`border-b border-zinc-50 dark:border-zinc-800/60 transition-colors ${
                        isMe ? "bg-green-50/50 dark:bg-green-900/10" : "hover:bg-zinc-50 dark:hover:bg-zinc-800/40"
                      }`}>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-lg bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-sm shrink-0">
                            {row.user.profile.avatar}
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-3.5 text-right text-zinc-500 font-mono text-xs">
                        {row.startWeight.toFixed(1)}
                      </td>
                      <td className="px-4 py-3.5 text-right font-bold text-zinc-900 dark:text-white">
                        {row.currentWeight.toFixed(1)}
                      </td>
                      <td className="px-4 py-3.5 text-right">
                        <span className={`font-bold ${row.lost >= 0 ? "text-green-500" : "text-red-500"}`}>
                          {row.lost >= 0 ? "−" : "+"}{Math.abs(row.lost).toFixed(1)} кг
                        </span>
                        <span className="block text-[10px] text-zinc-400">
                          {row.lostPct >= 0 ? "−" : "+"}{Math.abs(row.lostPct).toFixed(1)}%
                        </span>
                      </td>
                        ...
                        <div className="flex items-center gap-2 justify-end">
                          <div className="h-1.5 w-16 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-green-500"
                              style={{ width: `${Math.min(100, Math.max(0, row.goalPct))}%` }} />
                          </div>
